...
function readMultipie(target_size, multi_dir, target_dir)
%  rng('default');
if (multi_dir(end) == '/')
    multi_dir(end) = [];
end
...
if (nargin==2)
    target_dir = [multi_dir,'/prcoessed_',num2str(target_size)];
else
    if (target_dir(end) == '/')
        target_dir(end) = [];
    end
...
% img_list = dir(['../',multi_dir,'/*.jpg']);
label_list = dir(['../',multi_dir,'/meta/labels/*.mat']);
for i = 1:length(label_list)
% j = round(rand()*length(img_list));
% j = 59;
% for i = j:j
    fprintf('\r%.2f%% reading %s',i/length(label_list)*100, ['../',multi_dir,'/',label_list(i).name]);
    
    load(['../',multi_dir,'/meta/labels/',label_list(i).name]);
    
    dotloc = find(label_list(i).name == '.');
    
    comp = strsplit(label_list(i).name,'_');
    session = comp{2}(end);
    subject = comp{1};
    emo = comp{3};
    direction = comp{4};
%     light = comp{5};
    ...
    img_path = ['../', multi_dir,'/session',session,'/',subject,'/',emo,'/',direction(1:end-1),'_',direction(end),'/',label_list(i).name(1:dotloc-4),'.png'];
    img = imread(img_path);
    %img = imread(['../',multi_dir,'/',img_list(i).name]);
%     dotloc = find(img_list(i).name == '.');
%     pts = readPoints(['../',multi_dir,'/',img_list(i).name(1:dotloc),'pts']);

%%%%%    % may not have the right pts name here
    box = boundBox(pts);
    [h,w,~] = size(img);
    img = im2double(img);
...
[rh,rw,~] = size(result_img);
    ...
    imwrite(result_img, ['../',target_dir,'/',label_list(i).name(1:dotloc-1),'_',num2str(target_size),'.png']);
    writePoints(result_pts, ['../',target_dir,'/',label_list(i).name(1:dotloc-1),'_',num2str(target_size),'.pts']);
end
